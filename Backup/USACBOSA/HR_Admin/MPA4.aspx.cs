...
{
    public partial class MPA4 : System.Web.UI.Page
    {
        ...
        private void MemberNoshow()
        {
            try
            {
                ...
                dr = new WARTECHCONNECTION.cConnect().ReadDB("select m.surname,m.othernames,m.HomeAddr,m.companycode,m.entrance,m.companycode  from members m  where m.memberno ='" + txtMemberNo.Text + "'");
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
