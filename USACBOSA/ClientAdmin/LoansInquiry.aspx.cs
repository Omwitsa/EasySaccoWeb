    ...
    public partial class LoansInquiry : System.Web.UI.Page
    {
        ...
        private void LoadMemberDetails()
        {
            try
            {
                ...
                DR.Close();
                dr = new WARTECHCONNECTION.cConnect().ReadDB("select m.surname,m.othernames,M.CompanyCode , m.Dormant, m.Withdrawn, m.Archived,isnull(sum(s.TotalShares),0) as TotalShares from members m left outer join vwmembershares s on m.memberno=s.memberno where m.memberno='" + txtMemberNo.Text + "' and m.companycode='" + companycode + "' Group by  m.memberno,m.surname,m.othernames,M.CompanyCode , m.Dormant, m.Withdrawn, m.Archived");
                if (dr.HasRows)
                    while (dr.Read())
                    {
                        ...
                        dr1 = new WARTECHCONNECTION.cConnect().ReadDB("SELECT MemberNo, COUNT(MemberNo) AS LoanCount, ISNULL(SUM(Balance), 0) AS Balance FROM  VWMEMBERLOANS WHERE (MemberNo = '" + txtMemberNo.Text + "') and balance>0 GROUP BY MemberNo");
                        if (dr1.HasRows)
                            while (dr1.Read())
                            {
