...
namespace USACBOSA.LoansAdmin
{
    public partial class LoanApplications : System.Web.UI.Page
    {
        ...
        protected void txtLoanAmount_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                double LoanAmount = Convert.ToDouble(txtLoanAmount.Text);
                var query = "SELECT c.Capital FROM MEMBERS m INNER JOIN COMPANY c ON m.CompanyCode = c.CompanyCode WHERE m.MemberNo = '" + txtMemberNo.Text + "'";
                WARTECHCONNECTION.cConnect conCapital = new WARTECHCONNECTION.cConnect();
                dr1 = conCapital.ReadDB(query);
                if (dr1.HasRows)
                    while (dr1.Read())
                    {
                        double capital = Convert.ToDouble(dr1["Capital"].ToString());
                        if (LoanAmount > capital)
                        {
                            txtLoanAmount.Text = "";
                            WARSOFT.WARMsgBox.Show("Sorry, Currently you can only apply upto KES: " + capital);
                            return;
                        }
                    }
                dr1.Close(); dr1.Dispose(); dr1 = null; conCapital.Dispose(); conCapital = null;
            }
            catch (Exception ex)
            {
                WARSOFT.WARMsgBox.Show(ex.Message); return;
