<%@ Page Title="" Language="C#" MasterPageFile="~/CreditAdmin/CreditAdmin.Master" AutoEventWireup="true" CodeBehind="ExpenseEntry.aspx.cs" Inherits="USACBOSA.CreditAdmin.ExpenseEntry" %>
<%@ Register assembly="AjaxControlToolkit" namespace="AjaxControlToolkit" tagprefix="asp" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
<!--**********************************************************************************************//-->
<script src="../Scripts/JQuery.min.js" type="text/javascript"></script>
<script type="text/javascript" src="..\Scripts\jquery-1.3.1.min.js" > </script>

<script type="text/javascript">


    function PrintElem(elem) {

        Popup($(elem).html());

    }

    function Popup(data) {

        var mywindow = window.open('', 'my div', 'height=300,width=200');

        mywindow.document.write('<html><head><title>my div</title>');

        /*optional stylesheet*/ //mywindow.document.write('<link rel="stylesheet" href="main.css" type="text/css" />');

        mywindow.document.write('</head><body>');

        mywindow.document.write(data);

        mywindow.document.write('</body></html>');

        mywindow.document.close(); // necessary for IE >= 10

        mywindow.focus(); // necessary for IE >= 10

        mywindow.print();

        mywindow.close();

        return true;

    }

</script>
    <style type="text/css">
        .style30
        {
            width: 57px;
        }
        .style31
        {
            width: 106px;
        }
        .style32
        {
            height: 26px;
        }
        .style33
        {
            width: 57px;
            height: 26px;
        }
    </style>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
<%-- Add Reference to jQuery at Google CDN --%>

<script src="../Scripts/JQuery.min.js" type="text/javascript"></script>

<%-- Register the WebClientPrint script code --%>
        <asp:Label ID="Label5" runat="server" Text="Expenses Entry" Font-Bold="True" 
        Font-Size="14pt" ForeColor="#FF9900"></asp:Label> 
        <asp:ScriptManager ID="ScriptManager1" runat="server">
    </asp:ScriptManager>
        <hr style="color: Maroon" />
    <table style="width:100%;">
        <tr>
            <td>
                    <asp:Label ID="Label6" runat="server" Text="Company:" style="float: right" 
                        Width="77px"></asp:Label>
                    </td>
            <td>
                    <asp:DropDownList ID="DropDownList1" runat="server" Height="16px" Width="111px">
                    </asp:DropDownList>
                    </td>
            <td class="style30">
                    <asp:Label ID="Label26" runat="server" Text="Supplier:"></asp:Label>
            </td>
            <td colspan="2">
                    <asp:TextBox ID="txtNames" runat="server" Width="265px"></asp:TextBox>
                </td>
        </tr>
        <tr>
            <td>
                    <asp:Label ID="Label10" runat="server" Text="Cost Center:" 
                        style="float: right"></asp:Label>
                    </td>
            <td>
                    <asp:DropDownList ID="cboBankAC" runat="server" AutoPostBack="True" 
                        onselectedindexchanged="cboBankAC_SelectedIndexChanged" Height="22px" 
                        Width="110px">
                    </asp:DropDownList>
                </td>
            <td class="style30">
                <asp:Label ID="Label27" runat="server" Text="SubLedger"></asp:Label>
            </td>
            <td colspan="2">
                    <asp:TextBox ID="txtBankAC" runat="server" Width="265px"></asp:TextBox>
                </td>
        </tr>
        <tr>
            <td>
                    <asp:Label ID="Label11" runat="server" Text="Document No:" 
                        style="float: right"></asp:Label>
                    </td>
            <td>
                    <asp:DropDownList ID="cboPaymentMode" runat="server" Height="22px" 
                        Width="110px" AutoPostBack="True" 
                        onselectedindexchanged="cboPaymentMode_SelectedIndexChanged">
                        <asp:ListItem></asp:ListItem>
                        <asp:ListItem>Cheque</asp:ListItem>
                        <asp:ListItem>Mpesa</asp:ListItem>
                        <asp:ListItem>Bank Deposit</asp:ListItem>
                        <asp:ListItem>Dividends</asp:ListItem>
                        <asp:ListItem>Mobi Cash</asp:ListItem>
                    </asp:DropDownList>
                </td>
            <td class="style30">
                <asp:Label ID="Label28" runat="server" Text="Account:"></asp:Label>
            </td>
            <td colspan="2">
                    <asp:TextBox ID="TextBox3" runat="server"></asp:TextBox>
            </td>
        </tr>
        <tr>
            <td class="style32">
                    <asp:Label ID="Label15" runat="server" Text="Date:" 
                        style="float: right"></asp:Label>
                    </td>
            <td class="style32">
                         <asp:TextBox ID="txtContribDate" runat="server" Width="107px" Font-Size="8pt" 
                             Height="22px"></asp:TextBox>
                     <asp:CalendarExtender ID="txtContribDate_CalendarExtender2" Enabled="true" PopupButtonID="ImageButton2" format = "dd-MM-yyyy" runat="server" 
                                TargetControlID="txtContribDate">
                            </asp:CalendarExtender>
                    </td>
            <td class="style33">
                <asp:Label ID="Label29" runat="server" Text="Ref No:"></asp:Label>
            </td>
            <td colspan="2" class="style32">
                    <asp:TextBox ID="TextBox4" runat="server"></asp:TextBox>
            </td>
        </tr>
        <tr>
            <td>
                    <asp:Label ID="Label16" runat="server" Text="Bank Account No:" 
                        style="float: right"></asp:Label>
                </td>
            <td>
                    <asp:DropDownList ID="cboBankAC0" runat="server" AutoPostBack="True" 
                        onselectedindexchanged="cboBankAC_SelectedIndexChanged" Height="22px" 
                        Width="110px">
                    </asp:DropDownList>
                </td>
            <td class="style30">
                &nbsp;</td>
            <td colspan="2">
                    &nbsp;</td>
            <td colspan="2">
                    &nbsp;</td>
        </tr>
        <tr>
            <td>
                    <asp:Label ID="Label13" runat="server" Text="Receipt Amount:" 
                        style="float: right"></asp:Label>
                </td>
            <td>
                    <asp:TextBox ID="txtReceiptAmount" runat="server"
                        AutoPostBack="True" ontextchanged="txtReceiptAmount_TextChanged" 
                        Width="110px"></asp:TextBox>
                </td>
            <td class="style30">
                &nbsp;</td>
            <td class="style31">
                    &nbsp;</td>
            <td>
                &nbsp;</td>
            <td>
                &nbsp;</td>
        </tr>
        <tr>
            <td>
                    &nbsp;</td>
            <td>
                    <asp:CheckBox ID="Chckbxprintrcpt" runat="server" Checked="True" 
                        style="color: #000000" Text="AutoGenerate" />
                </td>
            <td class="style30">
                &nbsp;</td>
            <td>
                    &nbsp;</td>
            <td class="style31">
                    &nbsp;</td>
            <td>
                &nbsp;</td>
            <td>
                &nbsp;</td>
        </tr>
        <tr>
            <td>
                    <asp:Label ID="Label19" runat="server" Text="Amount:" 
                        style="float: right"></asp:Label>
                </td>
            <td>
                    <asp:TextBox ID="TextBox1" runat="server" Height="21px" Width="170px"></asp:TextBox>
                    <asp:Label ID="Label31" runat="server" style="font-weight: 700" Text="VAT"></asp:Label>
                </td>
            <td class="style30">
                    &nbsp;</td>
            <td colspan="2">
                    &nbsp;</td>
        </tr>
        <tr>
            <td>
     
                    <asp:Label ID="Label25" runat="server" Text="Total:" 
                        style="float: right"></asp:Label>
     
                </td>
            <td>
     
                    <asp:TextBox ID="txtEditedDistributedAmount" runat="server" AutoPostBack="True" 
                        ForeColor="#FF3300" ontextchanged="txtEditedDistributedAmount_TextChanged" 
                        Width="169px" Height="22px"></asp:TextBox>
                    <asp:Button ID="Button1" runat="server" Text="Add Expense" 
                        onclick="Button1_Click" />
                </td>
            <td class="style30">
                &nbsp;</td>
            <td colspan="4">
                &nbsp;
                    </td>
        </tr>
        <tr>
            <td>
     
                    <asp:Label ID="Label30" runat="server" Text="Narration"></asp:Label>
     
                </td>
            <td>
     
                    <asp:TextBox ID="TextBox2" runat="server" Height="22px" value="New Expense" 
                        Width="355px"></asp:TextBox>
                </td>
            <td class="style30">
                &nbsp;</td>
            <td colspan="4">
                &nbsp;</td>
        </tr>
        </table>
    <hr style="color: Maroon" />
    <asp:GridView ID="GridView2" runat="server" Width="893px" 
                        AutoGenerateSelectButton="True" Font-Size="8pt" 
                        onselectedindexchanged="GridView2_SelectedIndexChanged">
                        <FooterStyle BackColor="White" ForeColor="#000066" />
            <HeaderStyle BackColor="#009933" Font-Bold="True" ForeColor="White" />
            <PagerStyle BackColor="White" ForeColor="#000066" HorizontalAlign="Left" />
            <RowStyle ForeColor="#000066" />
                    </asp:GridView>
                     </asp:Content>
