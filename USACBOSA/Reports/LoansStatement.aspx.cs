    ...
    public partial class LoansStatement : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                da = new WARTECHCONNECTION.cConnect().ReadDB2("SELECT * from vwLoanStatement where memberno='" + MemberNo + "' order by datereceived");
                DataSet ds = new DataSet();
                //da.Fill(ds, "vwLoanStatement");
                ReportDocument rd = new ReportDocument();
                //rd.Load("C:\\Windows\\Temp\\LoansStatement.rpt");
                string path = Server.MapPath("~\\easysacco_reports\\LoansStatement.rpt");
                rd.Load(path);
                //rd.SetDataSource(ds.Tables[0]);
                rd.SetParameterValue("Pm-MEMBERS.MemberNo", MemberNo);
