...
{
    public partial class HR_Admin : System.Web.UI.MasterPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    DR = new WARTECHCONNECTION.cConnect().ReadDB("select UserName,usergroup,companycode from Useraccounts1 where UserLoginid='" + usrLoginID + "'");
                    if (DR.HasRows)
                        while (DR.Read())
                        {
                            this.Label2.Text = DR["UserName"].ToString();
                            this.Label4.Text = DR["usergroup"].ToString();
                            this.Label5.Text = DR["companycode"].ToString();
                        }
                    DR.Close();
                }
                catch (Exception ex)
