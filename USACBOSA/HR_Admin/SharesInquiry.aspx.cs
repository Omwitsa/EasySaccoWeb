    ...
    public partial class SharesInquiry : System.Web.UI.Page
    {
        ...
        private void LoadMemberDetails()
        {
            ...
            try
            {
                ...
                DR.Close();
                WARTECHCONNECTION.cConnect oSaccoMaster = new WARTECHCONNECTION.cConnect();
                dr = oSaccoMaster.ReadDB("select  m.surname,m.othernames,M.CompanyCode , m.Dormant, m.Withdrawn, m.Archived,isnull(sum(s.TotalShares),0) as TotalShares from members m left outer join vwmembershares s on m.memberno=s.memberno where m.memberno='" + txtMemberNo.Text + "' and m.companycode='" + companycode + "' Group by  m.memberno,m.surname,m.othernames,M.CompanyCode , m.Dormant, m.Withdrawn, m.Archived");
                if (dr.HasRows)
                    while (dr.Read())
                    {
