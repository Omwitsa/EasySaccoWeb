...
namespace USACBOSA.FinanceAdmin
{
    public partial class MPA_Paymentposting : System.Web.UI.Page
    {
        ...
        public DateTime duedate2 = DateTime.Today;
        ...
        private void LoadPendingPayments()
        {
            try
            {
                ...
                string query, query2, query3, query4, query5, myquery;
                ...
                query = "SELECT * FROM LOANS WHERE GroupCode IS NOT NULL AND Status = '1'";
                dr = new WARTECHCONNECTION.cConnect().ReadDB(query);
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        string MemberNo = " ";
                        string MemberNames = " ";
                        string groupCode = dr["GroupCode"].ToString().Replace("'", " ");
                        string LoanShareCode = dr["LoanNo"].ToString().Replace("'", " ");
                        DateTime DateApproved = Convert.ToDateTime(dr["ApplicDate"].ToString());
                        double AmountPayable = Convert.ToDouble(dr["LoanAmt"].ToString());
                        double Balance = Convert.ToDouble(dr["LoanAmt"].ToString());
                        string Purpose = dr["LoanCode"].ToString().Replace("'", " ");
                        string Narration = dr["LoanCode"].ToString().Replace("'", " ");
                        double Deductions = 0;
                        string isnnndadata = "set dateformat dmy Insert into PendingPayments(MemberNo,MemberNames,LoanShareCode,DateApproved,AmountPayable,Balance,Purpose,Narration,Deductions, GroupCode)values('" + MemberNo + "','" + MemberNames + "','" + LoanShareCode + "' ,'" + DateApproved + "','" + AmountPayable + "','" + Balance + "','Loan','" + Narration + "','" + Deductions + "', '"+ groupCode + "')";
                        new WARTECHCONNECTION.cConnect().WriteDB(isnnndadata);
                    }
                }
                dr.Close(); dr.Dispose(); dr = null;
                ...
                myquery = "select MemberNo,MemberNames,LoanShareCode,DateApproved,AmountPayable,Balance,Purpose,Narration,ISNULL(GroupCode, ' ') AS GroupCode from pendingpayments order by id asc";
        ...
        private void EffectLoanPayment(string Loanno, string chequeno, double pesaYaCheque, double balance, string ContraAcc, DateTime Dateissued, string StaffNo, string transactionNo, string lcode)
        {
            try
            {
                ...
                else
                {
                    ...
                    string query = "SELECT l.GroupCode, t.LoanAcc FROM LOANS l INNER JOIN LOANTYPE t ON l.LoanCode = t.LoanCode WHERE l.LoanNo = '"+ Loanno + "'";
                    dr1 = new WARTECHCONNECTION.cConnect().ReadDB(query);
                    if (dr1.HasRows)
                    {
                        while (dr1.Read())
                        {
                            ...
                            else
                            {
                                LoanAcc = dr1["LoanAcc"].ToString();
                                mMemberno = groupCode;
                                SaveGroupCapital(groupCode, pesaYaCheque);
                            }
                        }
                    }
                    dr1.Close(); dr1.Dispose(); dr1 = null;
        ...
        private void SaveGroupCapital(string groupCode, double pesaYaCheque)
        {
        ...
        protected void GridView1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                groupCode.Text = GridView1.SelectedRow.Cells[9].Text;

                if (string.IsNullOrEmpty(txtMemberNo.Text.ToString().Trim()))
                {
                ...
                if (string.IsNullOrEmpty(groupCode.Text.ToString().Trim()))
                {
