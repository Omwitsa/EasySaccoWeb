...
namespace USACBOSA.FinanceAdmin
{
    public partial class Journal_Posting : System.Web.UI.Page
    {
        ...
        System.Data.SqlClient.SqlDataReader dr, dr1, dr2, dr3, dr4, dr5, DR;
        ...
        protected void txtMemberNo_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                DR.Close();
                dr = new WARTECHCONNECTION.cConnect().ReadDB("select surname,othernames,HomeAddr,companycode  from members  where memberno ='" + txtMemberNo.Text.Trim() + "' and companycode='" + companycode + "'");
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
